...
st.title('📈 小山式・トリプル・フィルター・タイミング戦略分析アプリ')
...
st.sidebar.info("本アプリは、私個人が投資に使用するものであり、特定の金融商品を推奨するものではありません。表示される価格はあくまでテクニカル分析に基づく目安であり、利益を保証するものではありません。投資の最終的な判断はご自身の責任で行ってください。")